...
> **Note**: Column headers are case-insensitive and ignore spaces/underscores (e.g., "First Name", "firstname", "First_Name" are treated as identical).
...
| `Branch` | *(Resolved to ID)* | **Yes** | Name of existing Branch |
| `Department` | *(Resolved to ID)* | **Yes** | Name of existing Department |
| `Role` | *(Resolved to ID)* | **Yes** | Name of existing Role |
...
- `shift_type` (VARCHAR): Type (FULL_TIME, PART_TIME, etc.)
